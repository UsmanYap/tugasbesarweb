...
if (isset($_POST['submit'])) {
    ...
    if (empty($username)) {
        $errors['username'] = "username tidak boleh kosong sayangf !";
    }
    if (empty($password)) {
        $errors['password'] = "Password ya jangan kosong ya !";
    }
    if (empty($re_password)) {
        $errors['re_password'] = "ini jangan kosong juga ya say !";
    }
    if ($password !== $re_password) {
        $errors['matchPass'] = "konfirmasi nya salah la beb !";
    }

    $sql = "SELECT * FROM users WHERE username = '$username' ";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        $errors['existUser'] = "User nya telah ada !";
    }
...
<body>
    <form action="signup.php" method="post">
        ...
        <a href="login.php" class="ca">Udah punya akun kah?</a>
    </form>
</body>
