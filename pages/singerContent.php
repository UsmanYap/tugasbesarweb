...
if (isset($_GET['singerID'])){
    ...
    if (count($singer) > 0){
        ...
        $songsQuery = "SELECT songs.id, songs.title title,
                        songs.filePath audio, songs.imgPath img, singers.name singerName,
                        singers.id singerID
                    FROM songs
                    LEFT JOIN singers on singers.id = songs.singerID
                    WHERE singers.id = $singerID
                    ORDER BY songs.dataAdded DESC";
        $result2 = mysqli_query($cann, $songsQuery);
        ...
        <div class = "song" data = "<?php echo $song['id']; ?>">
            <div class = "info">
                ...
                <div class = "detail">
                    ...
                    <h5 data-singer = "<?php echo $song["singerID"]; ?>"><?php echo $song['singerName']; ?></5>
                </div>
            </div>
            <div class = "func">
